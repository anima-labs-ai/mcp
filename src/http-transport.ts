...
import { createSessionRegistry, type SessionRegistry, type SessionRegistryOptions } from "./session-registry.js";
import { createMcpRateLimiter, type McpRateLimiter, type McpRateLimiterOptions } from "./mcp-rate-limiter.js";
import { createCircuitBreaker, CircuitOpenError, type CircuitBreaker, type CircuitBreakerOptions } from "./circuit-breaker.js";
...
export function createMcpHttpServer(
	...
	const createServer_ = typeof serverFactory === "function" ? serverFactory : () => serverFactory;
		...
		if (req.method === "POST") {
			...
			if (options?.authenticate) {
				try {
					...
					metrics.authFailure();
					jsonError(res, authErr.status || 401, authErr.message || "Authentication failed");
					return;
				}
			}
			...
			const transport = new StreamableHTTPServerTransport({
				...
				onsessioninitialized: (sid: string) => {
					sessions.set(sid, { server: mcpServer, transport, apiKeyId, orgId });
